...
- 按正文 `h1`–`h6` 标题建立可勾选的层级目录；没有正文标题时回退为一个文档节点；
- 勾选父标题会导出该标题及其全部下级章节，父子重复选择会自动去重；
...
3. 可选：点击“读取文档”，按正文标题检查并勾选要导出的章节；默认全选。
4. 选择输出目录并点击“开始导出”。部分选择仍生成一个 Markdown，并按原文顺序合并；为保持上下文，会保留所选章节的祖先标题，但不会带入祖先标题的直属正文或未选中的兄弟章节。
5. 导出结果位于输出目录下的文档目录中。
