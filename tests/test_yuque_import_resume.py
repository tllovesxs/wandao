...
class YuqueImportResumeTests(unittest.TestCase):
    def test_parser_accepts_checkpoint_resume_and_retry_args(self) -> None:
        args = import_yuque.parse_args(
            [
                "--target-book-url",
                "https://www.yuque.com/demo/book",
                "--source-dir",
                "source",
                "--api-import-all",
                "--yes",
                "--checkpoint-file",
                "source/.wandao/yuque-import.sqlite",
                "--checkpoint-task-id",
                "task-1",
                "--resume",
                "--retry-failures",
    ...
    def test_item_stop_check_is_caught_by_import_loop(self) -> None:
        source = Path(import_yuque.__file__).read_text(encoding="utf-8")
